...
from base.models import Domain, Project, ProjectFlatpage
    ...
    def process_template_response(self, request, response):
        ...
        if context.get("project", None):
            ...
            if request.user.is_staff:
                ...
                context["the_projects"] = Project.approved_objects.filter(private=False)
        ...
        if context.get("ballots", None):
            try:
                context["the_project"] = context.get("ballots")[0].committee.project
        ...
        if context.get("entries", None):
            try:
                ...
                context["the_project"] = context.get("entries")[0].version.project
            except (KeyError, IndexError):
                pass

        if context.get("categories", None):
            try:
                context["the_project"] = context.get("categories")[0].project
            except (KeyError, IndexError):
                pass
        ...
        context["flatpages"] = FlatPage.objects.exclude(id__in=project_flatpage_ids)
            ...
            if domain in settings.VALID_DOMAIN:
                ...
                if custom_domain.role == "Project":
                    ...
                    is_different_project = (
                        Project.objects.filter(name__iexact=project_url_path)
                        .exclude(id=custom_domain.project.id)
                        .exists()
                    )

                    if (
                        request.path == url
                        or request.path == home_url
                        or project_url_path == ""
                        or is_different_project
                    ):
                        return redirect("project-detail", custom_domain.project.slug)
